...
public sealed class TcpApiModule : MonitoringApiModule
{
    ...
    private async Task HandleTcpStatusAsync(
        ...
        CancellationToken cancellationToken)
    {
        var result = ExtensionDeviceApi.GetTcpStatus(Runtime, deviceId.Trim());
        if (result is null)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var err = ExtensionDeviceApi.OpenTcpConnection(Runtime, deviceId.Trim(), config);
    ...
    private async Task HandleTcpDisconnectAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.CloseTcpConnection(Runtime, deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    {
        var config = ToConfig(configReq);
        var err = ExtensionDeviceApi.SetTcpConfig(Runtime, deviceId.Trim(), config);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpWriteAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.WriteTcpText(Runtime, deviceId.Trim(), data);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpWriteBinAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.WriteTcpBinary(Runtime, deviceId.Trim(), data);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        var (err, data) = ExtensionDeviceApi.ReadTcpAll(Runtime, deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpDiscardAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.DiscardTcpBuffers(Runtime, deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
