...
public sealed class SerialApiModule : MonitoringApiModule
{
    ...
    private async Task HandleSerialStatusAsync(
        ...
        CancellationToken cancellationToken)
    {
        var result = ExtensionDeviceApi.GetSerialStatus(Runtime, deviceId.Trim());
        if (result is null)
        {
    ...
    private async Task HandleSerialOpenAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.OpenSerialPort(Runtime, deviceId.Trim(), ToConfig(config));
    ...
    private async Task HandleSerialCloseAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.CloseSerialPort(Runtime, deviceId.Trim());
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleSerialConfigAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.SetSerialConfig(Runtime, deviceId.Trim(), ToConfig(config));
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleSerialWriteAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.WriteSerialText(Runtime, deviceId.Trim(), data);
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleSerialWriteBinAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.WriteSerialBinary(Runtime, deviceId.Trim(), data);
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleSerialReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        var (err, data) = ExtensionDeviceApi.ReadSerialAll(Runtime, deviceId.Trim());
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleSerialDiscardAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = ExtensionDeviceApi.DiscardSerialBuffers(Runtime, deviceId.Trim());
        if (err != SerialErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
