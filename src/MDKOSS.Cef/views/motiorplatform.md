...
- `src/MDKOSS.Core/server/monitorplatformpage.cs` → 读取 `views/monitorPlatform.html`
- `src/MDKOSS.Core/server/monitoringserver.cs`：`GET /monitorPlatform.html` 返回该 HTML

`MDKOSS.Cef/MDKOSS.Cef.csproj` 已包含 `views/**/*` 复制规则，无需改 csproj。
...
| 文件 | 角色 |
|---|---|
| `src/MDKOSS.Cef/views/motiorplatform.md` | 本设计说明 |
| `src/MDKOSS.Cef/views/monitorPlatform.html` | 页面实现（待开发） |
| `src/MDKOSS.Core/server/monitorplatformpage.cs` | 静态页加载器（待开发） |
| `src/MDKOSS.Core/core/mdev.cs` | `PlatformDevice` / `MPlatformKind` |
| `src/MDKOSS.Core/core/mdk.cs` | `ExecuteDeviceAction` |
| `src/MDKOSS.Core/server/monitoringserver.cs` | HTTP 路由 |
