...
public sealed class TcpApiModule : MonitoringApiModule
{
    ...
    private async Task HandleTcpStatusAsync(
        ...
        CancellationToken cancellationToken)
    {
        var result = Runtime.GetTcpStatus(deviceId.Trim());
        if (result is null)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var err = Runtime.OpenTcpConnection(deviceId.Trim(), config);
    ...
    private async Task HandleTcpDisconnectAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = Runtime.CloseTcpConnection(deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    {
        var config = ToConfig(configReq);
        var err = Runtime.SetTcpConfig(deviceId.Trim(), config);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpWriteAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = Runtime.WriteTcpText(deviceId.Trim(), data);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpWriteBinAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = Runtime.WriteTcpBinary(deviceId.Trim(), data);
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        var (err, data) = Runtime.ReadTcpAll(deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
    ...
    private async Task HandleTcpDiscardAsync(
        ...
        CancellationToken cancellationToken)
    {
        var err = Runtime.DiscardTcpBuffers(deviceId.Trim());
        if (err != TcpErrorCode.Ok)
        {
            await WriteErrorAsync(response, $"error_{err}", cancellationToken);
            return;
